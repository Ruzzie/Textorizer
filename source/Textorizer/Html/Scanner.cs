...
namespace Textorizer.Html; //todo: clean-up and refactor code so it is more 'structured', more readable blocks
...
internal static class Scanner
{
    ...
    public static readonly Token EOF_TOKEN = new Token(TokenType.Eof, 0,0, 0);
    ...
    private static Token CreateToken(TokenType tokenType, HtmlElementType htmlElementType, in SourceScanState state)
    {
        return new Token(tokenType
                       , state.StartIndex
                       , state.CurrentPos - state.StartIndex
                       , state.BlockLevel
                       , htmlElementType
                        );
    }
    ...
    SCAN_FOR_HTML_TAG_END:
              ...
              )
        {
            ...
            if (!char.IsWhiteSpace(current))
            {
                ...
                if (IsInQuote(current) &&
                    current == attrQuoteState) // when we are in an " or ' quote, and we found the matching closing quote
                {
        ...
        if (state.IsAtEnd())
        {
            // we are at EOF and still in a quote of an HTML tag
    ...
    RETURN_TOKEN:
        ...
        //: so return the token
        int tokenStringContentStartIndex = textValueRangeForTokenInSource.Start.Value;
        int tokenStringContentLength     = textValueRangeForTokenInSource.End.Value - tokenStringContentStartIndex;

        var tokenToReturn = new Token(currTokenType
                                    , tokenStringContentStartIndex
                                    , tokenStringContentLength
                                    , blockLevelForToken
                                    , htmlElementType);
